...
今日が記念日に該当するライバーを表示します。対象は周年・半周年に加えて、3桁以上のキリ番日数と特別キリ番日数を含みます。
...
同じライバーに複数の節目が重なる場合は、カードの主ラベルを1つ表示し、補助ラベルとして他の節目を小さく表示します。
...
- 左側にフィルター
- 右側上部に検索、並び替え、昇順/降順の操作
- 右側に一覧

スマホ:

- 上部に検索
- 並び替えは「五十音順 / デビュー日順 / 経過日数 / 記念日が近い順」を選べるセレクトまたはメニュー
- 昇順/降順は隣接するトグルボタンで切り替える
...
五十音順では、昇順は「あ」から「ん」、降順は「ん」から「あ」とします。デビュー日順では、昇順は古い順、降順は新しい順とします。
